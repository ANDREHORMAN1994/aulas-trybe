...
const sendMarsTemperature = (callbackAprove, callbackError) => {
  let percentage = Math.round(Math.random() * 100);
  return percentage >= 60 ?
   setTimeout(() => callbackAprove(getMarsTemperature()), messageDelay()) :
   setTimeout(() => callbackError(`Percentage of ${percentage}`), messageDelay())
}
